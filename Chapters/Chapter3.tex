...
The Adam optimizer \cite{kingma2017adammethodstochasticoptimization} was chosen for this study instead of the standard Stochastic Gradient Descent (SGD), despite SGD being commonly used for training deep learning models, particularly in long-tailed classification tasks \cite{menon2021longtaillearninglogitadjustment}.

Adam combines the benefits of momentum and adaptive learning rates, enabling faster convergence and more stable training, particularly in scenarios where the dataset is imbalanced or the model architecture is complex. CIFAR-100's long-tailed distribution presents significant optimization challenges, as tail classes can produce weaker gradient signals, potentially leading to slower convergence or poor generalization. Adam's adaptive learning rate mechanism mitigates these challenges by scaling updates based on the magnitude of recent gradients, ensuring that tail class gradients are not overshadowed by those of head classes.

Moreover, Adam requires less hyperparameter tuning compared to SGD, which often necessitates careful tuning of the learning rate and momentum to achieve optimal performance. This makes Adam particularly suitable for experiments involving multiple loss functions and model architectures, as it provides a consistent baseline for comparison without introducing additional variables.

While SGD with momentum remains the standard optimizer for many tasks, it may struggle in highly imbalanced settings without additional modifications such as weight decay or learning rate warm-up \cite{loshchilov2018fixing}. For this study, Adam's robustness and ease of use outweighed the potential benefits of using SGD, ensuring a more stable training process across all configurations.

\subsubsection{Fine-Tuning}
Transfer learning of weights from the larger ImageNet dataset can help overcome the problem of data scarcity in the long-tailed CIFAR-100 datasets \cite{cs231n,ye2023partialfinetuningsuccessorfinetuning,kandel2020}. Fine-tuning is another aspect of transfer learning, where parameters of a pre-trained model are trained on the new target data. In this study, none of the layers in the pretrained models were frozen during training, resulting in fine-tuning of all parameters, allowing the model to fully adapt its learned feature representations to the CIFAR-100 dataset \cite{ye2023partialfinetuningsuccessorfinetuning}. 

However, this approach comes with trade-offs. Fine-tuning all layers increases the risk of overfitting, especially when training on a smaller dataset. Additionally, the pretrained feature representations from ImageNet are optimized for general object recognition, and fully fine-tuning the network may lead to the loss of some of these general features. %Despite these risks, the experimental results indicate that full fine-tuning likely enhanced performance on tail classes, as the model could adjust its representations to emphasize these underrepresented categories.
