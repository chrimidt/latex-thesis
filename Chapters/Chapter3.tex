...
% Limitations:
% While ViT-B/16 has demonstrated strong performance, certain limitations should be noted. For datasets like CIFAR-100, with images of size 32x32, resizing to 224x224 is often necessary to align with the pretrained input resolution. This resizing can lead to:

% Loss of Fine-Grained Details: Interpolation during resizing may blur or distort small features, which are critical for distinguishing between similar classes.

% Artificial Features: Interpolation can introduce artifacts that the model may misinterpret as real patterns.

% Increased Computational Overhead: Larger image sizes result in a higher memory footprint and longer training times.

% Alternative strategies, such as using smaller patch sizes (e.g., 8x8) or advanced data augmentation techniques, could mitigate these drawbacks. Despite these limitations, ViT-B/16 remains a robust choice for exploring the applicability of transformer-based architectures in handling long-tailed datasets.
